...
public class QiblaActivity extends SherlockActivity {
	...
	private final SensorEventListener mListener = new SensorEventListener() {
		public void onSensorChanged(SensorEvent event) {
			// if (false) Log.d(TAG,"sensorChanged (" + event.values[0] + ", " +
	...
	public void calc2() {
		...
		double psi = 180.0
				...
				* Math.atan2(
						...
						Math.cos(phi) * Math.tan(phiK) - Math.sin(phi)
								* Math.cos(lambdaK - lambda));
		Q = 360 + psi;
	}
	...
	protected void onResume() {
		super.onResume();
		sensorManager.registerListener(mListener, mSensor,
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getTitle().equals("Info")) {
			...
			dialog.setMessage("-Arah kiblat Malaysia sepatutnya 29X.XXX\n\n" +
					"-Jika anggaran terlalu jauh, sila lock GPS anda terlebih dahulu \n\n" +
					"-Pusing telefon anda dalam bentuk 8 untuk calibrate GPS");
	...
	private class SampleView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (sValues != null) {
				canvas.rotate(-sValues[0]
						+ Float.parseFloat(Double.toString(Q)));
				// txtTest.setText(Float.toString(sValues[0]));
			}
			// int x=0; int y=0;
			// canvas.drawBitmap(resizedBitmap,x - resizedBitmap.getWidth() / 2,
			// y - resizedBitmap.getHeight(), null);
